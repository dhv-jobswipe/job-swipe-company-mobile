...
class InsertExperienceScreen extends StatefulWidget {
  final ProfileViewModel viewModel;

  InsertExperienceScreen({super.key, required this.viewModel});

  @override
  State<InsertExperienceScreen> createState() => _InsertExperienceScreenState();
}

class _InsertExperienceScreenState extends State<InsertExperienceScreen> {
  ...
  void onAddExperience(BuildContext context) {
    ...
    Future.delayed(Duration(seconds: 1), () async {
      if (_formKey.currentState!.validate()) {
        await widget.viewModel.insertExperience(
          onSuccess: () {
            check.fire();
            Future.delayed(Duration(seconds: 2), () {
              ...
              (e) => Future.delayed(Duration(seconds: 1), () async {
                await widget.viewModel
                    .getProfile()
                    .then((value) => context.pop('addExperience'));
              }),
            );
          },
          onFailure: (e) {
              ...
              child: Padding(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      TextFormField(
                        controller: widget.viewModel.addWorkPlaceController,
                        decoration: InputDecoration(labelText: 'Work Place'),
                      ),
                      TextFormField(
                        controller: widget.viewModel.addPositionController,
                        decoration: InputDecoration(labelText: 'Position'),
                      ),
                      TextFormField(
                        controller: widget.viewModel.addExpStartTimeController,
                        ...
                        onTap: () async {
                          ...
                          if (picked != null)
                            widget.viewModel.addExpStartTimeController.text =
                                DateFormat('dd-MM-yyyy').format(picked);
                        },
                        readOnly: true,
                      ),
                      TextFormField(
                        controller: widget.viewModel.addExpEndTimeController,
                        decoration: InputDecoration(
                        ...
                        onTap: () async {
                          final DateTime? picked = await showDatePicker(
                            context: context,
                            initialDate: widget.viewModel
                                .addExpEndTimeController.text.toInitialDateTime,
                            firstDate: DateTime(1900, 1),
                            lastDate: DateTime.now(),
                          );
                          if (picked != null)
                            widget.viewModel.addExpEndTimeController.text =
                                DateFormat('dd-MM-yyyy').format(picked);
                        },
                        readOnly: true,
                      ),
                      ...
                      Builder(
                        builder: (context) {
                          final value = context.select((ProfileViewModel vm) =>
                              vm.addSelectedSystemConstants);
                          return CustomDropdownButton(
                            placeholder: "Select Experience Type",
                            onChanged: (value) {
                              if (value != null) {
                                widget.viewModel.addSelectedSystemConstants =
                                    value;
                            ...
                            items: getIt
                                .get<AppData>()
                                .systemConstants[
                                    SystemConstantPrefix.EXPERIENCE_TYPE]!
                      ...
                      TextFormField(
                        controller: widget.viewModel.addExpNoteController,
