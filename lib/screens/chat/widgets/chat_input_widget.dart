...
class _ChatInputWidgetState extends State<ChatInputWidget> {
  ...
  Widget build(BuildContext context) {
    return CustomContainer(
      ...
      child: CustomFormField<MessageInputModel>(
                  ...
                  child: Wrap(
                    ...
                    children: currentMessage.files
                        ...
                        .map((entry) => CustomImage.avatar(
                              ...
                              file: entry.value,
                              fileBuilder: (file) {
                                if (!file.name.isImage && !file.name.isVideo) {
                                      ...
                                      onTap: () async {
                                        ...
                                        switch (e) {
                                          case InputMessageOptions.camera:
                                            var file = await ImagePickerHelper
                                                .takePhotoFromCamera(
                                                    context: context);
                                            if (file != null) {
                                              files.add(file);
                                            ...
                                            break;
                                          case InputMessageOptions.gallery:
                                            List<XFile> xfiles =
                                                await ImagePicker()
                                                    .pickMultiImage();
                                            files = xfiles
                                          ...
                                          case InputMessageOptions.file:
                                            files = await FileHelper.getFile(
                                                    context: context,
                                                    allowMultiple: true) ??
                                                [];
                                            break;
                  ...
                  Expanded(
                    child: CustomTextField(
                      ...
                      suffixIconConstraints:
                          const BoxConstraints(maxHeight: 0, maxWidth: 12),
                      onChanged: (value) {
                        changeState(currentMessage =
